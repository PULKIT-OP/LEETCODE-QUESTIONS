    ...
    vector<vector<int>> updateMatrix(vector<vector<int>>& mat) {
        ...
        while(!que.empty()){
            ...
            while(size--){
              ...
              // iterating over neighbours of parent cell
                for(auto &e : direction){
                    int new_x = x + e.first;
                    int new_y = y + e.second;
                  // if you get any neighbour which is -1 it means it's having 1 
                  // we know the steps to reach parent cell so just add 1 to it and update this current neighbour
                    if(new_x >= 0 && new_x < n && new_y >= 0 && new_y < m && sizeMat[new_x][new_y] == -1){
                        sizeMat[new_x][new_y] = sizeMat[x][y] + 1;    // updating neighbour by adding one step to parent cell
                        que.push({new_x, new_y});    // And now push this to q so that we can traveerse to its neighbour in future
                    }
                }
            }
        }
